...
if __name__ == '__main__':
    ...
    ead = Attack_EADL1(confidence=10, max_iterations=500)
    time_start = time.time()
    X_test_adv = ead.attack(model, X_test, Y_test_target_ll)
    dur_per_sample = (time.time() - time_start) / len(X_test_adv)

    # Evaluate the adversarial examples.
    print("\n---Statistics of EADL1 Attack (%f seconds per sample)" % dur_per_sample)
