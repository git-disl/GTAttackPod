...
if __name__ == '__main__':
    ...
    cw = Attack_CarliniLi(confidence=10, max_iterations=500)
    time_start = time.time()
    X_test_adv = cw.attack(model, X_test, Y_test_target_ml)
    dur_per_sample = (time.time() - time_start) / len(X_test_adv)

    # Evaluate the adversarial examples.
    print("\n---Statistics of CarliniLi Attack (%f seconds per sample)" % dur_per_sample)
