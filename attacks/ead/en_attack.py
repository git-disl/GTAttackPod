...
def wrap_to_ead_model(model, X, Y):
    image_size, num_channels = X.shape[1], X.shape[3]
    num_labels = Y.shape[1]
    input_shape = X.shape[1:]

    m = Model(inputs=model.layers[0].input, outputs=model.layers[-2].output)

    input_tensor = Input(shape=input_shape)
    scalar_layer = Lambda(lambda x: x + 0.5)(input_tensor)
    output_tensor = m(scalar_layer)

    model_logits = Model(inputs=input_tensor, outputs=output_tensor)

    model_wrapper = EADModelWrapper(model_logits,
                                    image_size=image_size, num_channels=num_channels, num_labels=num_labels)
